...
api_key_openai = st.secrets["OPEN_AI_KEY"]
...
MODEL_CONFIGS = {
    ...
    "GPT-3.5": {"name": "gpt-3.5-turbo", "cost_per_1k_input": 0.0005, "cost_per_1k_output": 0.0015},
    "Claude-3 Sonnet": {"name": "claude-3-5-sonnet-20241022", "cost_per_1k_input": 0.003, "cost_per_1k_output": 0.015}
}

def calculate_tokens_and_cost(input_text, output_text, model_config):
...
def perform_audit(iosa_checklist, input_text, selected_model):
    ...
    if "GPT" in selected_model:
        response = openai.chat.completions.create(
            model=MODEL_CONFIGS[selected_model]["name"],
            messages=[
    ...
    # Calculate metrics
    metrics = calculate_tokens_and_cost(
        user_prompt, 
        output_text, 
...
def main():
    ...
    if selected_section_1 and selected_section_2:
        ...
        if st.button("Perform Audit"):
            with st.spinner(f"Performing audit using {selected_model}..."):
                audit_result, metrics = perform_audit(chunk1, chunk2, selected_model)
                ...
                with st.expander("Performance Metrics"):
                    ...
                    st.write(f"Estimated Cost: ${metrics['total_cost']}")
